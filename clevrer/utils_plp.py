...
def plot_video_trajectories(ftr_ori, loc_dim_st=0, save_id=''):
    ...
    """
    ftr_loc = ftr_ori[:, :, loc_dim_st:loc_dim_st+2]
    loc = np.transpose(ftr_loc, [1, 2, 0]) # loc is in shape [25, 2, 4]
    # import pdb; pdb.set_trace()
    os.system('mkdir -p ' + save_id)
    # plot_video(loc, save_id)
    plot_video_new_op(loc, save_id)

def plot_video_new_op(loc, save_id):
    """
    ...
    colors = ['purple', 'blue', 'brown']  # Add more colors as needed
    # purple_traj = [(0.2734, 0.4933), (0.2681, 0.4985), (0.2695, 0.4965)]
    for t in range(loc.shape[0]):
        for i in range(loc.shape[-1]):
            # Apply rotation and flipping
            x, y = loc[t, 0, i], loc[t, 1, i]
            # rotated_x, rotated_y = -y, x  # Rotate 90 degrees
            flipped_x = 1 - x     # Horizontal flip
            flipped_y = 1 - y     # Horizontal flip
            new_x = flipped_x
            new_y = flipped_y
            # new_x = x
            new_y = y
            ...
            # plt.plot(new_x, new_y, 'o', markersize=10, color=colors[i % len(colors)])
            # if i == 0:
            #     plt.text(new_x, new_y, '3', color=(0, 0, 0), size='xx-large')
            #     plt.plot(new_x, new_y, 'o', markersize=10, color='green')
            # elif i == 2:
            #     plt.text(new_x, new_y, '1', color=(0, 0, 0), size='xx-large')
            #     plt.plot(new_x, new_y, 'o', markersize=10, color='green')
            # elif i == 1:
            #     # import random
            #     # new_x, new_y = random.sample(purple_traj, 1)[0]
            #     plt.text(new_x, new_y, '0', color=(0, 0, 0), size='xx-large')
            #     plt.plot(new_x, new_y, 'o', markersize=10, color='purple')
            #     # purple_traj.append((new_x, new_y))
            # else:
            #     plt.text(new_x, new_y, '0', color=(0, 0, 0), size='xx-large')
            #     plt.plot(new_x, new_y, 'o', markersize=10, color=colors[i % len(colors)])
            plt.text(new_x, new_y, str(i), color=(0, 0, 0), size='xx-large')
            plt.plot(new_x, new_y, 'o', markersize=10, color=colors[i % len(colors)])
