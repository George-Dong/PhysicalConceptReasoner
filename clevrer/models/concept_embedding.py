...
class ConceptEmbedding(nn.Module):
    ...
    def similarity_return_ref2query(self, query_list, identifier, feed_dict):
        # pdb.set_trace()
        ...
        concept = self.get_concept(identifier)
        ref2query_list = []
        attribute_list = [] # [obj_num][mapping dimension][a,b]
        for query_idx, query_item in enumerate(query_list):
            query_item_list = [m(query_item) for m in mappings]
            query_item_list = [(item / item.norm(1, dim = -1, keepdim = True)) for item in query_item_list]
            attribute_list.append(query_item_list)
            
        # TODO: here each item of query_item shoudn't be stacked together, should be calculated feature similarity respectively!!!
        # the following part should be re-written!
        from clevrer.utils import match_ref2query
        from scipy.optimize import linear_sum_assignment

        objects_match_list = []

        for j in range(len(attribute_list) - 1):
            single_attribute_list = []
            sim_mat = torch.matmul(attribute_list[0][0], attribute_list[j+1][0].t())

            linear_sum_sim_mat = torch.zeros((sim_mat.shape[0], sim_mat.shape[1])).to(sim_mat.device)            

            for k in range(len(attribute_list[0])):

                sim_mat = torch.matmul(attribute_list[0][k], attribute_list[j+1][k].t())
                linear_sum_sim_mat += sim_mat


            ## fence: if want to match base on 4 intermediate embedded features
                # # sim_mat = torch.matmul(query_list[0], query_list[j+1].t())
                ...
                # single_attribute_list.append(ref2query)
            # objects_match_list.append(single_attribute_list)
            ## fence: if want to match base on 4 intermediate embedded features


            # fence2: calculate sim matrix for a single v in concept
            cost_mat_np = -1 * linear_sum_sim_mat.cpu().detach().numpy()
            row_idx, col_idx = linear_sum_assignment(cost_mat_np) 
            ref_obj_num = min(linear_sum_sim_mat.shape[1], linear_sum_sim_mat.shape[0])
            # pdb.set_trace()
            ref2query = {}  
            for ref_idx in range(ref_obj_num):
                tar_obj_id, ref_obj_id = int(row_idx[ref_idx]), int(col_idx[ref_idx]) 
